...
(function($) {
	...
	var $current,
		...
		sHtml += '</ul>';
		$select.wrap('<div ' + (sId ? 'id="inp_slct-' + sId + '" ' : '')
			+ 'class="inp_slct '
		...
		$items.width(nWidth).css('width', oOptions.elSelectWidth).click(function() {
			var $li = $(this),
			...
			if ($('.inp_slct > ul.active').length>1) resetDropdown($('.inp_slct > ul.active').not($dropdown)[0]);
		...
		var $dropdown = $('.inp_slct > ul.active, .inp_slct > ul:focus');
		...
		if (o.type==='mouseleave' && !$dropdown.hasClass('active') && !$dropdown.data('clicked')) $dropdown = $('.inp_slct > ul.active');
